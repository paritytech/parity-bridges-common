...
pub(crate) mod tests {
	use super::*;
	use crate::mock::{custom_test_ext, validators_addresses, TestRuntime};
	...
	fn example_header_with_tx() -> Header {
	...
	fn verify_transaction_finalized_works() {
		custom_test_ext(example_header_with_tx(), validators_addresses(1)).execute_with(|| {
			let storage = BridgeStorage::<TestRuntime>::new();
			assert_eq!(
				verify_transaction_finalized(
					&storage,
					&example_tx(),
					example_header_with_tx().hash(),
					&vec![example_tx()]
				),
				true,
			);
	...
	fn verify_transaction_finalized_rejects_proof_with_missing_tx() {
		custom_test_ext(example_header_with_tx(), validators_addresses(1)).execute_with(|| {
			let storage = BridgeStorage::<TestRuntime>::new();
			assert_eq!(
				verify_transaction_finalized(&storage, &example_tx(), example_header_with_tx().hash(), &vec![],),
				false,
			);
		});
	}
	...
	fn verify_transaction_finalized_rejects_unknown_header() {
		custom_test_ext(Default::default(), validators_addresses(1)).execute_with(|| {
			let storage = BridgeStorage::<TestRuntime>::new();
			assert_eq!(
				verify_transaction_finalized(
					&storage,
	...
	fn verify_transaction_finalized_rejects_unfinalized_header() {
		custom_test_ext(example_header_with_tx(), validators_addresses(1)).execute_with(|| {
			FinalizedBlock::put((0, H256::default()));
