...
decl_storage! {
	trait Store for Module<T: Trait<I>, I: Instance = DefaultInstance> as MessageLane {
		/// Optional pallet owner. Pallet owner has a right to halt all pallet operations
		/// and then resume it. If it is `None`, then there are no direct ways to halt/resume
