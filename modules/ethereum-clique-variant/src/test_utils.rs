...
use bp_eth_clique::{
	...
	Address, Bloom, CliqueHeader, Receipt, SealedEmptyStep, H256, U256,
};
...
impl HeaderBuilder {
	...
	pub fn with_number(number: u64) -> Self {
		Self::with_parent(&CliqueHeader {
			number: number - 1,
			seal: vec![bp_eth_clique::rlp_encode(&(number - 1)).to_vec(), vec![]],
			..Default::default()
		})
	}

	/// Creates default header on top of given parent.
	pub fn with_parent(parent_header: &CliqueHeader) -> Self {
		...
		Self {
			header: CliqueHeader {
				...
				gas_limit: GAS_LIMIT.into(),
				seal: vec![bp_eth_clique::rlp_encode(&current_step).to_vec(), vec![]],
...
pub fn build_custom_header<F>(author: &SecretKey, previous: &CliqueHeader, customize_header: F) -> CliqueHeader
where
	F: FnOnce(CliqueHeader) -> CliqueHeader,
{
...
pub fn insert_header<S: Storage>(storage: &mut S, header: CliqueHeader) {
...
pub fn insert_dummy_header<S: Storage>(storage: &mut S, header: CliqueHeader) {
	storage.insert_header(HeaderToImport {
		context: storage.import_context(None, &header.parent_hash).unwrap(),
		is_best: true,
...
pub fn validators_change_receipt(parent_hash: H256) -> Receipt {
	use bp_eth_clique::{LogEntry, TransactionOutcome};
