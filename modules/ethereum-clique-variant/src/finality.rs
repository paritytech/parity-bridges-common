...
pub struct CachedFinalityVotes<Submitter> {
	...
	pub unaccounted_ancestry: VecDeque<(HeaderId, Option<Submitter>, CliqueHeader)>,
}
...
fn empty_steps_signers(header: &CliqueHeader) -> BTreeSet<Address> {
	header
		.empty_steps()
		.into_iter()
		.flatten()
