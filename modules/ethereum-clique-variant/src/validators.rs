...
use bp_eth_clique::{Address, CliqueHeader, HeaderId, LogEntry, Receipt, U256};
use sp_std::prelude::*;
...
pub enum ValidatorsConfiguration {
	...
	/// Validators change at header.number % epoch_length == 0
	Epoch(u64, ValidatorsSource),
}
...
impl<'a> Validators<'a> {
	...
	pub fn maybe_signals_validators_change(&self, header: &CliqueHeader) -> bool {
...
pub(crate) mod tests {
	...
	use crate::{BridgeStorage, CliqueVariantScheduledChange, Headers, ScheduledChanges, StoredHeader};
	...
	fn try_finalize_with_scheduled_change(scheduled_at: Option<HeaderId>) -> Option<ChangeToEnact> {
		run_test(TOTAL_VALIDATORS, |_| {
			...
			let scheduled_change = CliqueVariantScheduledChange {
