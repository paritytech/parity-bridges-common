...
pub use bp_eth_clique::signatures::secret_to_address;

use crate::validators::{ValidatorsConfiguration, ValidatorsSource};
use crate::{ChainTime, CliqueVariantConfiguration, Config, GenesisConfig as CrateGenesisConfig, PruningStrategy};
use bp_eth_clique::{Address, CliqueHeader, H256, U256};
...
parameter_types! {
	...
	pub TestCliqueVariantConfiguration: CliqueVariantConfiguration = test_clique_variant_config();
	pub TestValidatorsConfiguration: ValidatorsConfiguration = test_validators_config();
}

impl Config for TestRuntime {
	type CliqueVariantConfiguration = TestCliqueVariantConfiguration;
...
/// CliqueVariant configuration that is used in tests by default.
pub fn test_clique_variant_config() -> CliqueVariantConfiguration {
	CliqueVariantConfiguration {
		...
		extra_vanity: 32,
		extra_seal: 64 + 1, // 64 bytes ECDSA signature + 1 byte recovery id
	}
}
