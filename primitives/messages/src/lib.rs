...
use bp_runtime::{
	AccountIdOf, BasicOperatingMode, Chain, HashOf, OperatingMode, RangeInclusiveExt,
...
use target_chain::MessageDispatchResult;
...
/// State of the message dispatcher. Our code is generic over dispatcher, but we assume
/// that all dispatchers will share some traits. That's why we use this struct
/// directly, without making code generic over it. However it may be changed in the future.
///
/// This state is reported to the messages pallet by the `MessageDispatch::dispatch` call.
/// It is stored as a part of `InboundLaneData` and is proved back to the sending chain
/// during message delivery confirmation transaction, where it is then written to the
/// matching `OutboundLaneData`.
///
/// This data may be used by the pallets that are using the messages pallet to implement
/// backpressure mechanism on the sending chain like that:
///
/// (1) initially assume that all sent messages are stuck somewhere in the bridge;
///
/// (2) exponentially increase the message fee at every block. The exponential factor
///     may need to be lower than the same for other queues (e.g. XCMP) though. That's
///     because a message roundtrip is significantly larger when it is sent over bridge
///     vs when it is sent within a local consensus. But there should be some reasonable
///     interval during which we assume the message will be delviered and confirmed. If
///     there are no reports about queue status at the bridged side during that interval,
///     we may start increasing the fee with a larger factor;
///
/// (3) during the message delivery confirmation transaction, we receive the updated may
///     bridged queue state and may start to lower the fee.
#[derive(Encode, Decode, Default, Clone, RuntimeDebug, PartialEq, Eq, TypeInfo, MaxEncodedLen)]
pub struct DispatcherState {
	/// Number of the messages, that are not yet processed by the dispatcher. We assume that
	/// dispatchers that are processing messages immediately will always set this field to
	/// zero. If dispatcher is inserting some/all incoming messages to some queue, it will
	/// set this field to the queue length.
	pub unprocessed_messages: MessageNonce,
}
...
pub struct InboundLaneData<RelayerId> {
	...
	/// State of the message dispatcher at this side of the bridge. We use this field as
	/// a cheap way to report some dispatcher-related data back to the sending chain.
...
impl<RelayerId> InboundLaneData<RelayerId> {
		...
		RelayerId: MaxEncodedLen,
	{
		relayers_entries
			...
			.checked_add(DispatcherState::max_encoded_len())
	}
...
pub struct OutboundLaneData {
	...
	/// The state of the bridged dispatcher, received during the last message delivery
	/// confirmation transaction. May be `None` if we have never yet seen any confirmation
	/// transactions.
	pub bridged_dispatcher_state: Option<DispatcherState>,
}
