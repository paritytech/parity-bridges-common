...
impl OptimizationCallbacks {
	fn optimize<Header: HeaderT>(self, mut justification: GrandpaJustification<Header>) -> GrandpaJustification<Header> {
	...
	Header::Number: finality_grandpa::BlockNumberOps,
{
	...
	for (precommit_idx, signed) in justification.commit.precommits.iter().enumerate() {
		// if we have collected enough precommits, we probabably want to fail/remove extra precommits
