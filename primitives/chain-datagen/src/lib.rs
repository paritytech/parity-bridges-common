...
/// Number of bytes, included in the signed DataGen transaction apart from the encoded call itself.
///
/// Can be computed by subtracting encoded call size from raw transaction size.
pub const TX_EXTRA_BYTES: u32 = 103;

/// Maximum weight of single DataGen block.
///
...
/// Maximal number of unrewarded relayer entries in DataGen confirmation transaction.
pub const MAX_UNREWARDED_RELAYERS_IN_CONFIRMATION_TX: MessageNonce = 128;

/// Maximal number of unconfirmed messages in DataGen confirmation transaction.
pub const MAX_UNCONFIRMED_MESSAGES_IN_CONFIRMATION_TX: MessageNonce = 128;

/// The target length of a session (how often authorities change) on DataGen measured in of number of
/// blocks.
///
...
/// Maximal number of GRANDPA authorities at DataGen.
pub const MAX_AUTHORITIES_COUNT: u32 = 5;

/// Reasonable number of headers in the `votes_ancestries` on DataGen chain.
///
/// See [`bp-header-chain::ChainWithGrandpa`] for more details.
pub const REASONABLE_HEADERS_IN_JUSTIFICATON_ANCESTRY: u32 = 8;

/// Approximate average header size in `votes_ancestries` field of justification on DataGen chain.
///
/// See [`bp-header-chain::ChainWithGrandpa`] for more details.
pub const AVERAGE_HEADER_SIZE_IN_JUSTIFICATION: u32 = 256;

/// Approximate maximal header size on DataGen chain.
///
...
pub mod time_units {
	...
	/// Milliseconds between DataGen chain blocks.
	pub const MILLISECS_PER_BLOCK: u64 = 6000;
	/// Slot duration in DataGen chain consensus algorithms.
	pub const SLOT_DURATION: u64 = MILLISECS_PER_BLOCK;

	/// A minute, expressed in DataGen chain blocks.
	pub const MINUTES: BlockNumber = 60_000 / (MILLISECS_PER_BLOCK as BlockNumber);
	/// A hour, expressed in DataGen chain blocks.
	pub const HOURS: BlockNumber = MINUTES * 60;
	/// A day, expressed in DataGen chain blocks.
	pub const DAYS: BlockNumber = HOURS * 24;
}

/// Block number type used in DataGen.
pub type BlockNumber = u64;

/// Hash type used in DataGen.
pub type Hash = <BlakeTwoAndKeccak256 as HasherT>::Out;

/// Type of object that can produce hashes on DataGen.
pub type Hasher = BlakeTwoAndKeccak256;

/// The header type used by DataGen.
...
/// DataGen Hasher (Blake2-256 ++ Keccak-256) implementation.
...
decl_bridge_runtime_apis!(datagen, grandpa);
