...
// Adapted from `polkadot-sdk`'s `bridges/testing/zombienet-sdk-tests/build.rs`: when the
// `zombie-metadata` feature is enabled it ensures the subxt metadata `.scale` files exist,
// generating them from locally built runtime WASM blobs (building the `*-runtime` crates on demand).
// The upstream `substrate-build-script-utils` cargo-key generation is intentionally dropped so this
// works in a shallow polkadot-sdk checkout.
...
fn find_wasm(chain: &str) -> Option<PathBuf> {
	const PROFILES: [&str; 3] = ["release", "testnet", "debug"];
...
fn main() {
	if env::var("CARGO_FEATURE_ZOMBIE_METADATA").is_err() {
		debug_output!("zombie-metadata feature not enabled, no need to check metadata files.");
		return;
	}

	// Ensure we have the needed metadata files in place to run the bridge zombienet tests.
	let manifest_path = env::var("CARGO_MANIFEST_DIR").unwrap();
	const METADATA_DIR: &str = "metadata-files";
	// Relay chains and their bridge-hub / asset-hub system parachains for both sides of the
	// Rococo <> Westend bridge.
	const CHAINS: [&str; 6] = [
		"rococo",
		"westend",
		"bridge-hub-rococo",
