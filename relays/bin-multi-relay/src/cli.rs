...
pub enum RelayHeaders {
	...
	#[structopt(flatten)]
	GatewayCircuit(gateway_circuit::RelayHeaders),
}
...
impl RelayHeaders {
	...
	pub async fn run(self) -> anyhow::Result<()> {
		match self {
			...
			Self::GatewayCircuit(arg) => arg.run().await?,
		}
		Ok(())
	}
}
...
pub enum RelayMessages {
	...
	GatewayCircuit(gateway_circuit::RelayMessages),
}
...
pub enum InitBridge {
	...
	GatewayCircuit(gateway_circuit::InitBridge),
}
...
pub enum SendMessage {
	...
	GatewayCircuit(gateway_circuit::SendMessage),
}
...
pub enum EncodeCall {
	...
	GatewayCircuit(gateway_circuit::EncodeCall),
}
...
pub enum EncodeMessagePayload {
	...
	GatewayCircuit(gateway_circuit::EncodeMessagePayload),
}
...
pub enum EstimateFee {
	...
	GatewayCircuit(gateway_circuit::EstimateFee),
}
...
pub enum DeriveAccount {
	...
	GatewayCircuit(gateway_circuit::DeriveAccount),
}
...
impl AccountId {
	...
	/// Perform runtime checks of SS58 version and get Millau's AccountId.
	pub fn into_circuit(self) -> bp_circuit::AccountId {
		self.check_and_get("Circuit", circuit_runtime::SS58Prefix::get())
	}

	/// Perform runtime checks of SS58 version and get Millau's AccountId.
	pub fn into_gateway(self) -> bp_gateway::AccountId {
		self.check_and_get("Gateway", gateway_runtime::SS58Prefix::get())
	}
