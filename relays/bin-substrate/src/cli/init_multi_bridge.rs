		...
		match $bridge {
			InitBridgeName::GatewayToCircuit => {
				...
				fn encode_init_bridge(
					init_data: pallet_multi_finality_verifier::InitializationData<<Source as ChainBase>::Header>,
...
impl InitMultiBridge {
	...
	pub async fn run(self) -> anyhow::Result<()> {
		select_bridge!(self.bridge, {
			...
			let gateway_id = self.gateway_id.0;
					...
					Bytes(
						Target::sign_transaction(
							*target_client.genesis_hash(),
							&target_sign,
							transaction_nonce,
							encode_init_bridge(initialization_data, gateway_id),
						)
						.encode(),
					)
				},
