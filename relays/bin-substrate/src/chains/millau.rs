	...
	fn encode_call(call: &Call) -> anyhow::Result<Self::Call> {
		Ok(match call {
			...
			Call::Transfer { recipient, amount } => millau_runtime::Call::Balances(
				millau_runtime::BalancesCall::transfer { dest: recipient.raw_id(), value: amount.cast() },
			),
				...
				match *bridge_instance_index {
					bridge::MILLAU_TO_RIALTO_INDEX => {
						...
						millau_runtime::Call::BridgeRialtoMessages(
							millau_runtime::MessagesCall::send_message { lane_id: lane.0, payload, delivery_and_dispatch_fee: fee.cast() },
						)
					},
					_ => anyhow::bail!(
