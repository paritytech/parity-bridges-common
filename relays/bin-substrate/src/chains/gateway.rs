...
impl CliChain for Gateway {
	...
	fn encode_message(_message: encode_message::MessagePayload) -> Result<Self::MessagePayload, String> {
		Err("Sending messages from Gateway is not yet supported.".into())
	}
}
...
impl CliEncodeCall for Gateway {
	...
	fn encode_call(call: &Call) -> anyhow::Result<Self::Call> {
		log::warn!("gateway: encode call {:?}", call);
		anyhow::bail!("Unsupported target bridge pallet with instance index:")
		// Ok(match call {
