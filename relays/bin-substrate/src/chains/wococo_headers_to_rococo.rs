...
mod tests {
	...
	fn maximal_balance_decrease_per_day_is_sane() {
		// Rococo/Wococo GRANDPA pallet weights. They're now using Rialto weights => using
		// `RialtoWeight` is justified.
		//
		// Using Rialto runtime this is slightly incorrect, because `DbWeight` of Rococo/Wococo
		// runtime may differ from the `DbWeight` of Rialto runtime. But now (and most probably
		// forever) it is the same.
		type RococoGrandpaPalletWeights =
		...
		//
		// Worst case: we're submitting proof for every source header. Since we submit every header,
		// the number of headers in ancestry proof is near to 0 (let's round up to 2). And the
		// number of authorities is 1024, which is (now) larger than on any existing chain =>
		// normally there'll be ~1024*2/3+1 commits.
		...
		let single_source_header_submit_call_weight =
			RococoGrandpaPalletWeights::submit_finality_proof(
		...
		let single_source_header_tx_cost =
			bp_rococo::WeightToFee::calc(&single_source_header_submit_tx_weight);
		let maximal_expected_decrease =
			single_source_header_tx_cost * number_of_source_headers_per_day;
		assert!(
