...
use bp_beefy::{BeefyMmrLeafUnpackedOf, BeefySignedCommitmentOf, BeefyValidatorIdOf};
use bp_header_chain::{
...
use relay_substrate_client::{
	BlockNumberOf, Chain, ChainWithBeefy, ChainWithGrandpa, Client, Error as SubstrateError,
	HashOf, HeaderOf, Subscription,
};
...
/// BEEFY finality engine.
pub struct Beefy<C>(PhantomData<C>);

#[async_trait]
impl<C: Chain + ChainWithBeefy> Engine<C> for Beefy<C>
where
	C: std::fmt::Debug,
	BeefySignedCommitmentOf<C>: std::fmt::Debug + Send + Sync,
	Option<BeefyMmrLeafUnpackedOf<C>>: std::fmt::Debug + Send + Sync,
{
	const ID: ConsensusEngineId = bp_beefy::BEEFY_ENGINE_ID;
	type FinalityProof = BeefySignedCommitmentWithMmrLeaf<C>;
	type InitializationData = bp_beefy::InitializationData<BlockNumberOf<C>, BeefyValidatorIdOf<C>>;

	fn is_halted_key() -> StorageKey {
		bp_beefy::storage_keys::is_halted_key(C::WITH_CHAIN_BEEFY_PALLET_NAME)
	}

	fn is_initialized_key() -> StorageKey {
		bp_beefy::storage_keys::best_block_number_key(C::WITH_CHAIN_BEEFY_PALLET_NAME)
	}
	...
	async fn prepare_initialization_data(
		...
		let best_finalized_header_hash = client
			.best_finalized_beefy_hash()
			.await
			.map_err(|err| Error::RetrieveBestFinalizedHeaderHash(C::NAME, err))?;
		let best_finalized_header = client
			.header_by_hash(best_finalized_header_hash)
			.await
			.map_err(|err| Error::RetrieveHeader(C::NAME, best_finalized_header_hash, err))?;
		let current_validator_set_id: bp_beefy::ValidatorSetId = client
			.storage_value(
				bp_beefy::storage_keys::bridged::valdiator_set_id_storage_key(
					C::AT_CHAIN_BEEFY_PALLET_NAME,
				),
				Some(best_finalized_header_hash),
			)
			.await
			.map_err(|err| {
				Error::RetrieveAuthoritiesSetId(C::NAME, best_finalized_header_hash, err)
			})?
			.ok_or_else(|| {
				Error::RetrieveAuthoritiesSetId(
					C::NAME,
					best_finalized_header_hash,
					SubstrateError::MissingMandatoryStorageValue,
				)
			})?;
		let current_validators: Vec<BeefyValidatorIdOf<C>> = client
			.storage_value(
			...
			.await
			.map_err(|err| Error::RetrieveAuthorities(C::NAME, best_finalized_header_hash, err))?
			.ok_or_else(|| {
		...
		let next_validators: Vec<BeefyValidatorIdOf<C>> = client
			.storage_value(
				bp_beefy::storage_keys::bridged::next_validators_storage_key(
					C::AT_CHAIN_BEEFY_PALLET_NAME,
				),
				Some(best_finalized_header_hash),
			)
			.await
			.map_err(|err| {
				Error::RetrieveNextAuthorities(C::NAME, best_finalized_header_hash, err)
			})?
			.ok_or_else(|| {
				Error::RetrieveNextAuthorities(
					C::NAME,
					best_finalized_header_hash,
					SubstrateError::MissingMandatoryStorageValue,
				)
			})?;
		Ok(bp_beefy::InitializationData {
			is_halted: false,
			best_beefy_block_number: *best_finalized_header.number(),
			current_validator_set: (current_validator_set_id, current_validators),
			next_validator_set: (current_validator_set_id.saturating_add(1), next_validators),
		})
	}
}

/// Finality proof, used by the BEEFY engine.
#[derive(Clone, Debug, Decode, Encode)]
pub struct BeefySignedCommitmentWithMmrLeaf<C: Chain + ChainWithBeefy> {
	/// Signed BEEFY commitment.
	pub commitment: BeefySignedCommitmentOf<C>,
}

impl<C: Chain + ChainWithBeefy> FinalityProof<BlockNumberOf<C>>
	for BeefySignedCommitmentWithMmrLeaf<C>
where
	C: std::fmt::Debug,
	BeefySignedCommitmentOf<C>: std::fmt::Debug + Send + Sync,
	Option<BeefyMmrLeafUnpackedOf<C>>: std::fmt::Debug + Send + Sync,
{
	fn target_header_number(&self) -> BlockNumberOf<C> {
		self.commitment.commitment.block_number
	}
}
