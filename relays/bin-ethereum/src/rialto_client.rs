...
pub trait SubstrateHighLevelRpc {
	...
	/// Returns the best finalized Ethereum block that Substrate runtime knows of.
	async fn best_ethereum_finalized_block(&self) -> RpcResult<EthereumHeaderId>;
	/// Returns or not transactions receipts are required for Ethereum header submission.
	async fn ethereum_receipts_required(&self, header: SubstrateEthereumHeader) -> RpcResult<bool>;
	/// Returns or not the given Ethereum header is known to the Substrate runtime.
