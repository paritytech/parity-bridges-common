	...
	C: MessageLaneTargetClient<P>,
{
	...
	) -> Result<(TargetHeaderIdOf<P>, RangeInclusive<P::MessageNonce>, P::MessagesProcessingProof), Self::Error> {
		self.client.prove_messages_processing(at_block).await.map(|(at_block, proof)| (at_block, nonces, proof))
	}
}
