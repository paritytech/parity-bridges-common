...
//! Circuit chain specification for CLI.
...
impl CliEncodeCall for Circuit {
	...
	fn encode_call(call: &Call) -> anyhow::Result<Self::Call> {
		println!("circuit: encode call {:?}", call);
		Ok(match call {
			...
			Call::BridgeSendMessage {
				...
				bridge::CIRCUIT_TO_GATEWAY_INDEX => {
...
impl CliChain for Circuit {
	...
	type MessagePayload =
		MessagePayload<bp_circuit::AccountId, bp_gateway::AccountSigner, bp_gateway::Signature, Vec<u8>>;
	...
	fn encode_message(message: encode_message::MessagePayload) -> Result<Self::MessagePayload, String> {
		match message {
			...
			encode_message::MessagePayload::Call { mut call, mut sender } => {
				...
				encode_call::preprocess_call::<Source, Target>(&mut call, bridge::CIRCUIT_TO_GATEWAY_INDEX);
