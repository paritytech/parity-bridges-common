...
mod tests {
	...
	fn should_encode_raw_message() {
		...
		let encode_message = EncodeMessage::from_iter(vec!["encode-message", "CircuitToGateway", "raw", msg]);
	...
	fn should_encode_remark_with_size() {
		...
		assert_eq!(
			format!("{:?}", hex),
			"0x01000000e88514000000000002d43593c715fdd31c61141abd04a99fd6822c8558854ccde39a5684e7a56da27d3c040130000000000000000000000000"
		);
	}
}
