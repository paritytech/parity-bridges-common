...
mod tests {
	...
	fn should_derive_accounts_correctly() {
		// given
		let gateway = "5sauUXUfPjmwxSgmb3tZ5d6yx24eZX4wWJ2JtVUBaQqFbvEU";
		let circuit = "752paRyW1EGfq9YLTSSqcSJ5hqnBDidBmaftGhBo8fy6ypW9";

		// when
		let (gateway_parsed, gateway_derived) = derive_account_cli("GatewayToCircuit", gateway);
		let (circuit_parsed, circuit_derived) = derive_account_cli("CircuitToGateway", circuit);
		let (circuit2_parsed, circuit2_derived) = derive_account_cli("CircuitToGateway", gateway);

		// then
		assert_eq!(format!("{}", gateway_parsed), gateway);
		assert_eq!(format!("{}", circuit_parsed), circuit);
		assert_eq!(format!("{}", circuit2_parsed), circuit);

		assert_eq!(
			format!("{}", gateway_derived),
			"73gLnUwrAdH4vMjbXCiNEpgyz1PLk9JxCaY4cKzvfSZT73KE"
		);
		assert_eq!(
			format!("{}", circuit_derived),
			"5rpTJqGv1BPAYy2sXzkPpc3Wx1ZpQtgfuBsrDpNV4HsXAmbi"
		);
		assert_eq!(circuit_derived, circuit2_derived);
	}
}
