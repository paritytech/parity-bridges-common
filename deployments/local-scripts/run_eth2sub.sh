...
RUST_LOG=runtime=trace,rpc=debug,txpool=trace,basic_authorship=trace unbuffer ./rialto-bridge-node --execution=Native --dev -d substrate.db 2>&1 | unbuffer -p gawk '{ print strftime("Substrate: [%Y-%m-%d %H:%M:%S]"), $0 }' | unbuffer -p tee substrate.log&
sleep 10
