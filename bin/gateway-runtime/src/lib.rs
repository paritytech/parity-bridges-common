...
//! The Gateway runtime. This can be compiled with `#[no_std]`, ready for Wasm.
...
pub use pallet_bridge_grandpa::Call as BridgeGrandpaCircuitCall;
pub use pallet_bridge_messages::Call as MessagesCall;
pub use pallet_substrate_bridge::Call as BridgeCircuitCall;
...
/// An index to a block.
pub type BlockNumber = bp_gateway::BlockNumber;

/// Alias to 512-bit hash when used in the context of a transaction signature on the chain.
pub type Signature = bp_gateway::Signature;

/// Some way of identifying an account on the chain. We intentionally make it equivalent
/// to the public key of our transaction signing scheme.
pub type AccountId = bp_gateway::AccountId;
...
pub type Balance = bp_gateway::Balance;

/// Index of a transaction in the chain.
pub type Index = u32;

/// A hash of some data used by the chain.
pub type Hash = bp_gateway::Hash;

/// Hashing algorithm used by the chain.
pub type Hashing = bp_gateway::Hasher;
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("gateway-runtime"),
	impl_name: create_runtime_str!("gateway-runtime"),
	authoring_version: 1,
	spec_version: 1,
	impl_version: 1,
...
impl frame_system::Config for Runtime {
	...
	type BlockWeights = bp_gateway::BlockWeights;
	/// The maximum length of a block (in bytes).
	type BlockLength = bp_gateway::BlockLength;
...
type GatewayPoA = pallet_bridge_eth_poa::Instance1;
impl pallet_bridge_eth_poa::Config<GatewayPoA> for Runtime {
	type AuraConfiguration = gateway_poa::BridgeAuraConfiguration;
	type FinalityVotesCachingInterval = gateway_poa::FinalityVotesCachingInterval;
	type ValidatorsConfiguration = gateway_poa::BridgeValidatorsConfiguration;
	type PruningStrategy = gateway_poa::PruningStrategy;
	type ChainTime = gateway_poa::ChainTime;
...
type GatewayCurrencyExchange = pallet_bridge_currency_exchange::Instance1;
impl pallet_bridge_currency_exchange::Config<GatewayCurrencyExchange> for Runtime {
	type OnTransactionSubmitted = ();
	type PeerBlockchain = gateway_poa::GatewayBlockchain;
...
impl pallet_bridge_dispatch::Config for Runtime {
	...
	type Call = Call;
	type CallFilter = ();
	type EncodedCall = crate::circuit_messages::FromCircuitEncodedCall;
	type SourceChainAccountId = bp_circuit::AccountId;
	type TargetChainAccountPublic = MultiSigner;
	type TargetChainSignature = MultiSignature;
	type AccountIdConverter = bp_gateway::AccountIdConverter;
}
...
parameter_types! {
	pub const MinimumPeriod: u64 = bp_gateway::SLOT_DURATION / 2;
}
...
parameter_types! {
	pub const ExistentialDeposit: bp_gateway::Balance = 500;
...
parameter_types! {
	pub const Period: BlockNumber = bp_gateway::SESSION_LENGTH;
...
impl pallet_substrate_bridge::Config for Runtime {
	type BridgedChain = bp_circuit::Circuit;
}
...
parameter_types! {
	...
	// `IdentityFee` is used by Gateway => we may use weight directly
	pub const GetDeliveryConfirmationTransactionFee: Balance =
		bp_gateway::MAX_SINGLE_MESSAGE_DELIVERY_CONFIRMATION_TX_WEIGHT as _;
	pub const RootAccountForPayments: Option<AccountId> = None;
}

pub(crate) type WithCircuitMessagesInstance = pallet_bridge_messages::DefaultInstance;
impl pallet_bridge_messages::Config for Runtime {
	type Event = Event;
	type WeightInfo = pallet_bridge_messages::weights::RialtoWeight<Runtime>;
	type Parameter = circuit_messages::GatewayToCircuitMessagesParameter;
	type MaxMessagesToPruneAtOnce = MaxMessagesToPruneAtOnce;
	type MaxUnrewardedRelayerEntriesAtInboundLane = MaxUnrewardedRelayerEntriesAtInboundLane;
	type MaxUnconfirmedMessagesAtInboundLane = MaxUnconfirmedMessagesAtInboundLane;

	type OutboundPayload = crate::circuit_messages::ToCircuitMessagePayload;
	type OutboundMessageFee = Balance;

	type InboundPayload = crate::circuit_messages::FromCircuitMessagePayload;
	type InboundMessageFee = bp_circuit::Balance;
	type InboundRelayer = bp_circuit::AccountId;

	type AccountIdConverter = bp_gateway::AccountIdConverter;

	type TargetHeaderChain = crate::circuit_messages::Circuit;
	type LaneMessageVerifier = crate::circuit_messages::ToCircuitMessageVerifier;
	...
	type SourceHeaderChain = crate::circuit_messages::Circuit;
	type MessageDispatch = crate::circuit_messages::FromCircuitMessageDispatch;
}
...
construct_runtime!(
	...
	{
		BridgeGatewayPoA: pallet_bridge_eth_poa::<Instance1>::{Module, Call, Config, Storage, ValidateUnsigned},
		BridgeKovan: pallet_bridge_eth_poa::<Instance2>::{Module, Call, Config, Storage, ValidateUnsigned},
		BridgeGatewayCurrencyExchange: pallet_bridge_currency_exchange::<Instance1>::{Module, Call},
		BridgeKovanCurrencyExchange: pallet_bridge_currency_exchange::<Instance2>::{Module, Call},
		BridgeCircuit: pallet_substrate_bridge::{Module, Call, Storage, Config<T>},
		BridgeCircuitGrandpa: pallet_bridge_grandpa::{Module, Call, Storage},
		BridgeDispatch: pallet_bridge_dispatch::{Module, Event<T>},
		BridgeCircuitMessages: pallet_bridge_messages::{Module, Call, Storage, Event<T>},
...
impl_runtime_apis! {
	...
	impl bp_circuit::CircuitHeaderApi<Block> for Runtime {
		fn best_blocks() -> Vec<(bp_circuit::BlockNumber, bp_circuit::Hash)> {
			BridgeCircuit::best_headers()
		}

		fn finalized_block() -> (bp_circuit::BlockNumber, bp_circuit::Hash) {
			let header = BridgeCircuit::best_finalized();
			(header.number, header.hash())
		}

		fn incomplete_headers() -> Vec<(bp_circuit::BlockNumber, bp_circuit::Hash)> {
			BridgeCircuit::require_justifications()
		}

		fn is_known_block(hash: bp_circuit::Hash) -> bool {
			BridgeCircuit::is_known_header(hash)
		}

		fn is_finalized_block(hash: bp_circuit::Hash) -> bool {
			BridgeCircuit::is_finalized_header(hash)
		}
	}

	impl bp_circuit::CircuitFinalityApi<Block> for Runtime {
		fn best_finalized() -> (bp_circuit::BlockNumber, bp_circuit::Hash) {
			let header = BridgeCircuitGrandpa::best_finalized();
			(header.number, header.hash())
		}

		fn is_known_header(hash: bp_circuit::Hash) -> bool {
			BridgeCircuitGrandpa::is_known_header(hash)
		}
	}
	...
	impl bp_currency_exchange::RialtoCurrencyExchangeApi<Block, exchange::EthereumTransactionInclusionProof> for Runtime {
		fn filter_transaction_proof(proof: exchange::EthereumTransactionInclusionProof) -> bool {
			BridgeGatewayCurrencyExchange::filter_transaction_proof(&proof)
		}
	}
	...
	impl bp_circuit::ToCircuitOutboundLaneApi<Block, Balance, ToCircuitMessagePayload> for Runtime {
		fn estimate_message_delivery_and_dispatch_fee(
			_lane_id: bp_messages::LaneId,
			payload: ToCircuitMessagePayload,
		) -> Option<Balance> {
			estimate_message_dispatch_and_delivery_fee::<WithCircuitMessageBridge>(
				&payload,
				WithCircuitMessageBridge::RELAYER_FEE_PERCENT,
			).ok()
		}
		...
		fn messages_dispatch_weight(
			...
			(begin..=end).filter_map(|nonce| {
				let encoded_payload = BridgeCircuitMessages::outbound_message_payload(lane, nonce)?;
				let decoded_payload = circuit_messages::ToCircuitMessagePayload::decode(
					&mut &encoded_payload[..]
				).ok()?;
		...
		fn latest_received_nonce(lane: bp_messages::LaneId) -> bp_messages::MessageNonce {
			BridgeCircuitMessages::outbound_latest_received_nonce(lane)
		}

		fn latest_generated_nonce(lane: bp_messages::LaneId) -> bp_messages::MessageNonce {
			BridgeCircuitMessages::outbound_latest_generated_nonce(lane)
		}
	}

	impl bp_circuit::FromCircuitInboundLaneApi<Block> for Runtime {
		fn latest_received_nonce(lane: bp_messages::LaneId) -> bp_messages::MessageNonce {
			BridgeCircuitMessages::inbound_latest_received_nonce(lane)
		}

		fn latest_confirmed_nonce(lane: bp_messages::LaneId) -> bp_messages::MessageNonce {
			BridgeCircuitMessages::inbound_latest_confirmed_nonce(lane)
		}

		fn unrewarded_relayers_state(lane: bp_messages::LaneId) -> bp_messages::UnrewardedRelayersState {
			BridgeCircuitMessages::inbound_unrewarded_relayers_state(lane)
		}
	}
	...
	impl frame_benchmarking::Benchmark<Block> for Runtime {
		fn dispatch_benchmark(
			...
			impl MessagesConfig<WithCircuitMessagesInstance> for Runtime {
				fn maximal_message_size() -> u32 {
					messages::source::maximal_message_size::<WithCircuitMessageBridge>()
				}
				...
				fn prepare_outbound_message(
				...
				) -> (circuit_messages::ToCircuitMessagePayload, Balance) {
					...
					let message = ToCircuitMessagePayload {
						spec_version: 0,
				...
				fn prepare_message_proof(
				...
				) -> (circuit_messages::FromCircuitMessagesProof, Weight) {
					use crate::circuit_messages::{Circuit, WithCircuitMessageBridge};
					...
					let circuit_account_id: bp_circuit::AccountId = Default::default();
					let (gateway_raw_public, gateway_raw_signature) = ed25519_sign(
						&call,
						&circuit_account_id,
					);
					let gateway_public = MultiSigner::Ed25519(sp_core::ed25519::Public::from_raw(gateway_raw_public));
					let gateway_signature = MultiSignature::Ed25519(sp_core::ed25519::Signature::from_raw(
						gateway_raw_signature,
					));

					let make_circuit_message_key = |message_key: MessageKey| storage_keys::message_key::<
						Runtime,
						<Circuit as ChainWithMessages>::MessagesInstance,
					>(
						&message_key.lane_id, message_key.nonce,
					).0;
					let make_circuit_outbound_lane_data_key = |lane_id| storage_keys::outbound_lane_data_key::<
						<Circuit as ChainWithMessages>::MessagesInstance,
					>(
						&lane_id,
					).0;
					let make_circuit_header = |state_root| bp_circuit::Header::new(
						0,
						Default::default(),
						state_root,
						Default::default(),
					...
					prepare_message_proof::<WithCircuitMessageBridge, bp_circuit::Hasher, Runtime, (), _, _, _>(
						params,
						make_circuit_message_key,
						make_circuit_outbound_lane_data_key,
						make_circuit_header,
						call_weight,
				...
				fn prepare_message_delivery_proof(
				...
				) -> circuit_messages::ToCircuitMessagesDeliveryProof {
					use crate::circuit_messages::{Circuit, WithCircuitMessageBridge};
					use bridge_runtime_common::{
						messages::ChainWithMessages,
					...
					prepare_message_delivery_proof::<WithCircuitMessageBridge, bp_circuit::Hasher, Runtime, (), _, _>(
						params,
						...
						|state_root| bp_circuit::Header::new(
							0,
							Default::default(),
							state_root,
							Default::default(),
			...
			add_benchmark!(
				...
				MessagesBench::<Runtime, WithCircuitMessagesInstance>
			);
...
/// Circuit account ownership digest from Gateway.
///
/// The byte vector returned by this function should be signed with a Circuit account private key.
/// This way, the owner of `gateway_account_id` on Gateway proves that the 'circuit' account private key
/// is also under his control.
pub fn circuit_account_ownership_digest<Call, AccountId, SpecVersion>(
	circuit_call: &Call,
	gateway_account_id: AccountId,
	circuit_spec_version: SpecVersion,
) -> sp_std::vec::Vec<u8>
where
	Call: codec::Encode,
...
mod tests {
	...
	fn ensure_gateway_message_lane_weights_are_correct() {
		...
		pallet_bridge_messages::ensure_weights_are_correct::<Weights>(
			...
			bp_gateway::ADDITIONAL_MESSAGE_BYTE_DELIVERY_WEIGHT,
			bp_gateway::MAX_SINGLE_MESSAGE_DELIVERY_CONFIRMATION_TX_WEIGHT,
		);

		let max_incoming_message_proof_size = bp_circuit::EXTRA_STORAGE_PROOF_SIZE.saturating_add(
			messages::target::maximal_incoming_message_size(bp_gateway::max_extrinsic_size()),
		);
		...
		let max_incoming_inbound_lane_data_proof_size = bp_messages::InboundLaneData::<()>::encoded_size_hint(
			bp_gateway::MAXIMAL_ENCODED_ACCOUNT_ID_SIZE,
			bp_circuit::MAX_UNREWARDED_RELAYER_ENTRIES_AT_INBOUND_LANE as _,
		)
		.unwrap_or(u32::MAX);
