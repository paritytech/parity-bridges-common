...
impl MessageBridge for WithRialtoMessageBridge {
	...
	fn this_weight_to_balance(weight: Weight) -> bp_millau::Balance {
		<crate::Runtime as pallet_transaction_payment::Trait>::WeightToFee::calc(&weight)
	}

	fn bridged_weight_to_balance(weight: Weight) -> bp_rialto::Balance {
		// we're using the same weights in both chains now
		<crate::Runtime as pallet_transaction_payment::Trait>::WeightToFee::calc(&weight)
	}

	fn this_chain_balance_to_bridged_chain_balance(this_balance: bp_millau::Balance) -> bp_rialto::Balance {
