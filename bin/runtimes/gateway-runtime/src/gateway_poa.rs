...
//! Configuration parameters for the Gateway PoA chain.
...
/// Aura engine configuration for Gateway chain.
...
/// Validators configuration for Gateway PoA chain.
...
/// Genesis validators set of Gateway PoA chain.
...
/// Genesis header of the Gateway PoA chain.
///
...
/// Gateway PoA headers pruning strategy.
///
...
/// The Gateway PoA Blockchain as seen by the runtime.
pub struct GatewayBlockchain;

impl InclusionProofVerifier for GatewayBlockchain {
	type Transaction = RawTransaction;
	type TransactionInclusionProof = EthereumTransactionInclusionProof;

	fn verify_transaction_inclusion_proof(proof: &Self::TransactionInclusionProof) -> Option<Self::Transaction> {
		let is_transaction_finalized =
			crate::BridgeGatewayPoA::verify_transaction_finalized(proof.block, proof.index, &proof.proof);
