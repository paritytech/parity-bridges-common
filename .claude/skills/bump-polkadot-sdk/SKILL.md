...
```bash
cd <POLKADOT_SDK_PATH>
cargo build -p bridge-hub-rococo-runtime --release
cargo build -p bridge-hub-westend-runtime --release
```
...
## CI alternative: validate the bump PR on GitHub

Steps 9-15 (both zombienet test suites) are also automated as a manually-triggered GitHub Action:
`.github/workflows/bridge-zombienet-tests.yml`.

After opening the bump PR, run it from **Actions -> "Bridge Zombienet Tests" -> Run workflow**, selecting the
PR branch and the `suite` (`both` / `rococo-westend` / `polkadot-kusama`). It builds `substrate-relay` from the
PR plus the polkadot-sdk binaries (using the hash pinned in `Cargo.lock`) and runs the same `run-test.sh` flows
described below. Prefer this for PR validation; use the local steps below for debugging or when iterating offline.
...
From the local polkadot-sdk repo `<POLKADOT_SDK_PATH>`, build each binary one by one:

```bash
cd <POLKADOT_SDK_PATH>
cargo build -p polkadot --features fast-runtime --release
```
...
**IMPORTANT: Run each `cargo build` command one at a time (sequentially, not in parallel). These are large release builds and running them concurrently can exhaust system resources.**

#### Polkadot binaries (without fast-runtime)

From `<POLKADOT_SDK_PATH>`, build each binary one by one:
...
Then build the chain-spec-generator:

```bash
cd <FELLOWS_RUNTIMES_PATH>
cargo build -p chain-spec-generator --no-default-features --features fast-runtime,polkadot,kusama,bridge-hub-kusama,bridge-hub-polkadot,asset-hub-kusama,asset-hub-polkadot --release
```
